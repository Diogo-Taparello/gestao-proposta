...
class Proposta extends BaseController
{
    ...
    public function all()
    {
        ...
        if( isset($data['titulo']) && $data['titulo'] != '')
            $propostaModel->like('produto', $data['titulo'], 'both');

        if( isset($data['status']) && $data['status'] != '')
            $propostaModel->where('status', $data['status']);

        if( isset($data['origem']) && $data['origem'] != '')
            $propostaModel->where('origem', $data['origem']);
        ...
        $propostaModel->limit( $limit, $page);
        $proposta = $propostaModel->findAll();
    ...
    public function add()
    {
        ...
        $insertId = $propostaModel->getInsertID();

        if( !isset($insertId) || !$insertId )
            return $this->failNotFound('Erro ao cadastrar nova proposta');

        $auditoriaModel = new AuditoriaModel;
        $auditoriaModel->insert(array('proposta_id' => $insertId, 'actor' => 'user:1', 'evento' => 'CREATED', 'payload' => json_encode($data)));
    ...
    public function change_status($id, $new_status)
    {
        ...
        $propostaModel = new PropostaModel;
        $proposta = $propostaModel->select('status')->find($id);
        ...
        $propostaModel = new PropostaModel;
        $propostaModel->update($id, array('status' => $new_status));
        
        $auditoriaModel = new AuditoriaModel;
        $auditoriaModel->insert(array('proposta_id' => $id, 'actor' => 'user:1', 'evento' => 'STATUS_CHANGED', 'payload' => json_encode(array('status' => $proposta['status'], 'new_status' => $new_status))));
    ...
    public function auditoria($id)
    {
        ...
        $auditoriaModel = new AuditoriaModel;
        $auditorias = $auditoriaModel->where('proposta_id', $id)->find();
